...
function coremark_build() {
    ...
    cd $BENCH_DIR
    common_build $SIM $ARCH $MODE *.c -I$EXTRA_DIR -DITERATIONS=1000 -DPERFORMANCE_RUN -DHAS_STDIO -DFLAGS_STR $EXTRA_DIR/core_portme.c -I$BENCH_DIR
    cd - > /dev/null
}
...
function cv32e40p_run() {
    ...
    if [[ "$PRINT" == "1" ]]
    then
        timeout --foreground $TIMEOUT $TESTBENCH "+firmware=cv32e40p.elf.hex" $EXTRA_ARGS > >(tee cv32e40p_out.txt) 2> >(tee cv32e40p_err.txt)
